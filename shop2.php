...
if (isset($_POST['add_to_cart'])) {
    ...
    $select_cart = mysqli_query($conn, "SELECT * FROM `cart` WHERE product_id = '$product_id'") or die(mysqli_error($conn));
    if (mysqli_num_rows($select_cart) > 0) {
        ...
        $insert_query = "INSERT INTO `cart` (product_id, name, image, quantity, price) VALUES ('$product_id', '{$product['product_name']}', '{$product['product_image']}', '$quantity', '{$product['product_price']}')";
  ...
  <meta name="keywords" content="bootstrap, bootstrap4" />
	...
	<body>
		...
		<div class="untree_co-section product-section before-footer-section">
            ...
            <img src="adminpanel/images/<?php echo $item['product_image']; ?>"
                alt="<?php echo $item['product_name']; ?>">
            <h3><?php echo $item['product_name']; ?></h3>
            <p>Price: <?php echo $item['product_price']; ?> USD</p>
            <form method="POST" action="">
                <input type="hidden" name="product_id" value="<?php echo $item['product_id']; ?>">
                <!-input type="number" name="quantity" value="1" min="1" max="10" required >
                <input type="hidden" name="add_to_cart"><span class="icon-cross">
